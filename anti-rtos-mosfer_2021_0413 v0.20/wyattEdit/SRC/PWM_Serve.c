...
*/
static void SetPWM_Duty_DownDir(float32 Duty_Cycle) 
{
		...
		//Enable shadow register transfer action, compare register will be updated.  
		CCU6_T12_Str_En();
}
...
test reulst:
		...
		Duty_Cycle:100~0,higer duty, higher speed
*/
 static void SetPWM_Duty_UpDir(float32 Duty_Cycle)
{
		uint16 PeriodTick=0,Tick=0;
...
/*
**
** According real window object condition to set PWM driver function
** COLOCKWISE is motor rotate direction,if it is 1,that means  real window test result
** is matched to the setting we make.if 0 , that means test result is opposite to the pre-setting
*/

void DutySet_RealUpDir(float32 Duty_Cycle)
{
	#if COLOCKWISE
		SetPWM_Duty_UpDir(Duty_Cycle); 
	#else
		SetPWM_Duty_DownDir(Duty_Cycle); 
	#endif

}

void DutySet_RealDownDir(float32 Duty_Cycle)
{
